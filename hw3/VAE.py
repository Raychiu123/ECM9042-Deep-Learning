...
parser.add_argument('--outf', default='VAEsamples/', help='folder to output images and model checkpoints')
...
class VAE(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc11 = nn.Linear(n * 8 * 8, opt.hidden_size)
        self.fc12 = nn.Linear(n * 8 * 8, opt.hidden_size)
        self.fc2 = nn.Linear(opt.hidden_size, n * 8 * 8)

        self.deconv1 = nn.Sequential(nn.ConvTranspose2d(n,n,kernel_size=4,stride=2,padding=1),
                                 nn.BatchNorm2d(n),
                                 nn.ReLU())
        # 14 x 14
        self.deconv2 = nn.Sequential(nn.ConvTranspose2d(n,3,kernel_size=4,stride=2,padding=1),
                                 nn.Sigmoid())
        # 28 x 28
    ...
    def decoder(self, x):
        out = self.fc2(x)
        out = self.deconv1(out.view(x.size(0), 64, 8, 8))
        # opt.batchSize x n x 7 x 7
...
def sample(epoch):
    ...
    vutils.save_image(fake.data.resize_(opt.batchSize,3,opt.imageSize,opt.imageSize),
...
torch.save(model.state_dict(), '%s/vae.pth' % (opt.outf))
...
np.savetxt(opt.outf + "v_loss.txt", v_loss)
np.savetxt(opt.outf + "v_eps.txt", v_eps)
